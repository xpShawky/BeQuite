...
| 3 | **Confidence surfacing** | Every claim in reports tagged verified/inferred/assumed | Trust calibration for vibe-handoff users | users can't tell guess from fact | tags in all reports | extend anti-hallucination | ✅ | none | low | none | V1 |
...
| 5 | **Professional Expert Mode (alias)** | Named composition: `deep` + anti-hallucination strict + regulated doctrine + evidence ledger mandatory | Asked for by user; capability exists as composition — needs only a NAME + docs row | "I want maximum-rigor mode" | mode alias docs | docs-level alias, NOT a 5th mode | ✅ | none | low (avoid mode-matrix bloat) | n/a | V1 — decide naming with user |
