...
## Now (last edited: 2026-05-10, end of v0.7.0)

- **Active feature:** `BeQuite v1.0.0` (the build of BeQuite itself).
- **Active phase:** `phase-3` per `state/project.yaml::build_phases` — Reproducibility + economics (covers v0.7.0 + v0.7.1 + v0.8.x).
- **Active sub-version:** v0.7.0 just tagged. Next: **v0.7.1** — Signed receipts (ed25519 keypair on init + sign-at-emission + `bequite verify-receipts` validator).
- **Last green sub-version:** `v0.7.0` (Reproducibility receipts — chain-hashed JSON receipts + emitter + store + validate-chain + replay-check + cost roll-ups + 10-test integration suite all passing).
- **17 tags total** at this snapshot: `v0.1.0` `v0.1.1` `v0.1.2` `v0.2.0` `v0.2.1` `v0.3.0` `v0.4.0` `v0.4.1` `v0.4.2` `v0.4.3` `v0.5.0` `v0.5.1` `v0.5.2` `v0.5.3` `v0.6.0` `v0.6.1` `v0.7.0`. Real git counts (post-v0.7.0): ~22 commits, ~170+ tracked files, ~29k lines added net.
...
## Eight operational modules now in place
...
## Four working Python modules (runnable today from local checkout)
...
v0.7.0 just shipped. Continuing to **v0.7.1 — Signed receipts (ed25519)** per the main plan:

1. `cli/bequite/receipts_signing.py` — ed25519 keypair generation on `bequite init` (private key in `.bequite/.keys/private.pem`, gitignored; public key in `.bequite/keys/public.pem`, committed).
2. Sign each receipt at emission: append `signature` field (base64-encoded ed25519 signature over the canonical-JSON content_hash).
3. `bequite verify-receipts` — validates each receipt's signature against the project's public key + walks chain.
4. Tampered-receipt test: edit a receipt's body byte → signature fails verification.
5. Refresh `cli/bequite/receipts.py::Receipt` to include optional `signature` field (additive-only schema bump).

After v0.7.1: v0.8.0 (multi-model routing live: AiProvider adapters for Anthropic + OpenAI + Google + DeepSeek + Ollama with cost ceiling enforcement) → v0.8.1 (live pricing fetch + 24h cache + offline fallback) → v0.9.0 (3 example projects: bookings-saas + ai-tool-wrapper + tauri-note-app) → v0.9.1 (e2e harness) → v0.10.0 (auto-mode state machine + safety rails + heartbeat) → v0.10.1+ per main plan.
...
## Next 5 things I'll do (after this v0.7.0 commit)

1. v0.7.1 — ed25519 signing on init + sign at emission + `bequite verify-receipts` validator + integration test for tampered-body rejection. Commit + tag.
2. v0.8.0 — Multi-model routing live (AiProvider adapters: Anthropic, OpenAI, Google, DeepSeek, Ollama). Cost ceiling enforcement via stop-cost-budget hook. Receipts emit with each model invocation. Commit + tag.
3. v0.8.1 — Live pricing fetch (24h cache, offline fallback). Surface vendor-pricing-changed warnings. Commit + tag.
4. v0.9.0 — Three example projects (bookings-saas Next/Hono/Supabase + ai-tool-wrapper CLI + tauri-note-app desktop) with full seven-phase walks executed. Commit + tag.
5. v0.9.1 — E2E test harness (seven-phase walk + auto-mode + doctrine-loading) + `examples-e2e.yml` CI workflow. Commit + tag.

## Heartbeat (auto-mode)

- Last heartbeat: 2026-05-10 (v0.7.0 close).
- First receipt: emittable now via `python -m bequite.receipts emit ...` from any BeQuite-managed project; first auto-mode receipt lands when v0.10.0 wires phase transitions.
- Last commit (pending): `feat(v0.7.0): Reproducibility receipts — chain-hashed JSON receipts + emitter + store + chain validation + replay + roll-ups + 10-test integration suite`.
