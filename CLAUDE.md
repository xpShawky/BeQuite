...
## Current spec: v3.0.0-alpha.25 — Skills-first workflow law + per-task execution profile + post-phase verify (on the alpha.24 base)

- **60 active slash commands + 1 deprecated alias** (`.claude/commands/`) — alpha.24 adds M3 `/bq-hooks` (manage opt-in safety hooks: status/enable/disable/test) · C12 `/bq-automation` · C13 `/bq-local-business` · C14 `/bq-brand-kit` · C15 `/bq-community` · C16 `/bq-recording` · C17 `/bq-start` (+ /bq-offer business-system modes · /bq-handoff workflow-export · /bq-release template · Course Architect); alpha.23 adds C11 `/bq-offer`; alpha.22 adds C3 `/bq-reference` · C4 `/bq-knowledge` · C5 `/bq-course` · C6 `/bq-pain-radar` · C7 `/bq-integrate` · C8 `/bq-proposal`; alpha.19 added `/bq-writing-dna` + `/bq-skill-audit`. **Catalog IDs (alpha.22):** display-only W0–W5 / N / O / C / M IDs in `.bequite/commands/COMMAND_ID_MAP.md` — files are NEVER renamed (`docs/architecture/COMMAND_NUMBERING_AND_ORDERING_STRATEGY.md`).
- **31 skills** (alpha.24 adds `bequite-automation-engineer` (merged from legacy skill/); alpha.22 adds `bequite-localization-rtl` + `bequite-guard-pass` + `bequite-orchestrator` (global brain — orchestration update); alpha.21 added `bequite-frontier-reasoning-coach`)
- **Global Orchestration (alpha.22 orchestration update)** — `bequite-orchestrator` skill + `.bequite/state/ORCHESTRATION_MAP.md` = source of truth on conflict/confusion/missing-capability; auto mode = 15-step anti-skip sequence (`AUTO_MODE_RULES.md`); mandatory System Design Risk Check for risky domains (`SYSTEM_DESIGN_REASONING_STANDARD.md`); context compaction at 40/60/75/85% (`CONTEXT_COMPACTION_RULES.md`); small-model tiers A/B/C (`LOW_COST_MODEL_RULES.md` — gap-narrowing, not magic); per-agent setup `docs/runbooks/INSTALL_FOR_OTHER_AGENTS.md`
...
| Cross-agent compatibility | `docs/architecture/CROSS_AGENT_COMPATIBILITY_STRATEGY.md` · `docs/runbooks/USING_BEQUITE_OUTSIDE_CLAUDE_CODE.md` · `docs/specs/AGENT_COMPATIBILITY_MATRIX.md` |
| Roadmap ledger (done/pending/parked/alpha.23) | `.bequite/tasks/REMAINING_ROADMAP_TASKS.md` · `ALPHA_22_TASK_CHECKLIST.md` |
...
| Skills (30) | `.claude/skills/bequite-*/SKILL.md` |
| Orchestration (alpha.22 update) | `bequite-orchestrator` · `.bequite/state/{ORCHESTRATION_MAP,AUTO_MODE_RULES,CONTEXT_COMPACTION_RULES,LOW_COST_MODEL_RULES}.md` · `docs/architecture/{BEQUITE_ORCHESTRATION_MODEL,AUTO_MODE_ORCHESTRATION_STRATEGY,SYSTEM_DESIGN_REASONING_STANDARD,LOW_COST_MODEL_EXECUTION_STRATEGY,CONTEXT_COMPACTION_STRATEGY}.md` |
...
17. **Skills-first + per-task execution profile + post-phase verify** (alpha.25). (a) **Skills-first, global:** every command — workflow AND capability (course/presentation/reference/offer/automation/brand-kit/…) AND maintenance — selects and announces suitable skills (the `Skill Selection:` block) BEFORE any plan/content/edit/tool-call for the task; acting before announcing skills is a contract violation (trivial reads now/help/explain exempt). `/bq-discover`+`/bq-research` select skills and may recommend a VERIFIED external skill on a real gap (safe path, user-approved install only — never auto-install); `/bq-plan` records per-phase skills; `/bq-assign` records per-task skills. (b) **Per-task Execution Profile:** each task/phase carries a recommended model + tier · effort (quick/standard/high/max) · confidence % (`docs/architecture/TASK_EXECUTION_PROFILE.md`). BeQuite *recommends* a model; it never switches models (reports any reroute); never-give-small list forces tier A. (c) **Post-phase verify gate:** after implementing any phase/task, select verification skills and run its tests + verification with evidence before advancing; failure → `/bq-fix`, not the next phase.
