...
## 2026-06-13 - alpha.25 HOTFIX: install-bequite.ps1 BOM regression broke `irm|iex` (claude-opus-4-8)

**Action:** Right after the alpha.25 push, user re-ran `irm|iex` and hit `lightweight : ... not recognized` with a `﻿#` BOM on line 1 - a REPEAT of the hotfix #2 BOM class. Root cause: the alpha.25 version-bump script wrote install-bequite.ps1 with Python `utf-8-sig`, re-prepending the UTF-8 BOM that hotfix #2 had stripped; `ParseFile` and `[ScriptBlock]::Create` both mask a leading BOM, so the alpha.25 "ps1 OK" check passed on a broken file. Fix: stripped the 3-byte BOM (scripts/install-bequite.ps1 is pure ASCII, no BOM again; no other change). REPRODUCED the exact failure via `[Text.Encoding]::UTF8.GetString([IO.File]::ReadAllBytes(...)) | iex` (faithful irm|iex byte path), then RE-VERIFIED clean on that same path in an isolated temp dir: no `lightweight`/`CommandNotFound`; 61 cmd files / 31 skills installed. Sharpened the standing release check (byte-level BOM assert + real `iex` run, not ParseFile/ScriptBlock) + MISTAKE_MEMORY entry. Counts unchanged (60/31). Model: opus-4-8.

## 2026-06-13 - v3.0.0-alpha.25: skills-first workflow law + execution profile + post-phase verify (claude-opus-4-8)

**Action:** User asked to make skill-selection happen BEFORE every command (global, incl. course/presentation/etc.), record skills in discover/research/plan/assign, add per-task model+effort+confidence, and add a post-phase test+verify step. Implemented as workflow law: contract step 4 (skills-first, global, capability commands included, before any action), step 8 (execution profile reference), step 9 (post-phase verify gate); new TASK_EXECUTION_PROFILE.md (recommended model+tier/effort/confidence; honest "recommends not switches" + never-give-small tier gate); AUTO_SKILL_ROUTING_STRATEGY extended (skills-first global rule + per-phase/per-task skills + research-discovered-external-skill SAFE path: verify+10Q+inspectable+user-approved-install-only); CLAUDE.md core rule 17 + installer-written project CLAUDE.md skills-first rule (the "system prompt"). Command appends: discover/research (skills-first + external-skill discovery), plan (per-phase profile), assign (per-task profile: skills+model+effort+confidence), implement/feature (post-phase verify gate). ORCHESTRATION_MAP s15. Bumped alpha.24->alpha.25 (installers + VERSION + README + commands.md + CLAUDE.md + banner counts 60/31); both installers parse OK. No new commands/skills. Model: opus-4-8 (recommends-not-switches discipline now codified for tasks).
...
## 2026-06-13 - alpha.24 HOTFIX #2: BOM/non-ASCII in install-bequite.ps1 (claude-opus-4-8)

**Action:** After the param() fix, user re-ran `irm|iex` and hit a 2nd error: `﻿<#` token + `.SYNOPSIS`/`Copies`/`No`/`By` "not recognized as cmdlet". Diagnosed: UTF-8 BOM (added alpha.19 for file-exec em-dash decoding) is returned by irm as the first char, glues to `<#`, breaks the comment-block opener -> every help line executed as a command; plus 26 lines had em/en-dashes. Fix: stripped BOM, replaced all em/en-dashes with ASCII hyphen, replaced the `<# #>` help block with `#` line comments -> pure ASCII (0 bytes >127), no BOM. ASCII means file execution is also safe without the BOM, resolving the alpha.19 vs irm|iex tension. VERIFIED END-TO-END (not just compile): `Get-Content -Raw | iex` with $env:BEQUITE_FROM_LOCAL in a temp dir (no network) -> "v3.0.0-alpha.24 installed", 60 cmd files / 31 skills / scaffold incl .bequite/automation. Also fixed stale installer banner counts (46->59, 27->31). Lessons in MISTAKE_MEMORY + EVIDENCE_LOG: irm|iex installer .ps1 must be pure ASCII + no BOM + no leading block comment; test with the real iex byte-path, not ParseFile. Model: opus-4-8.

## 2026-06-13 - alpha.24 HOTFIX: ps1 irm|iex install path (claude-opus-4-8)

**Action:** User hit a real bug running `irm install-bequite.ps1 | iex` on Windows: "Unexpected attribute CmdletBinding / Unexpected token param". Diagnosed: the documented primary Windows install path has been broken since early alpha (param()/[CmdletBinding()] block is invalid under Invoke-Expression; only bash curl|bash worked). My prior installer verification used ParseFile (file-syntax) which never exercised the iex path. Fix: removed param()/[CmdletBinding()], parse args manually from $args + env fallbacks (BEQUITE_FROM_LOCAL/BEQUITE_FORCE); file execution with -FromLocal/-Force preserved. Verified with [ScriptBlock]::Create on the raw file (the exact iex path) -> OK, and ParseFile -> 0 errors. Logged the verification-gap lesson to MISTAKE_MEMORY + EVIDENCE_LOG; new standing check = iex-compile test on every installer-touching release. bash installer audited (clean: args via while-over-$@, no $0/BASH_SOURCE/read hazards). Model: opus-4-8 (user /model set it).

## 2026-06-13 - v3.0.0-alpha.24: selected-V2 build + P1 maintenance (claude-opus-4-8)

**Model note:** user text said "fable 5" but the /model command set claude-opus-4-8; reported the conflict before executing and ran on the harness-selected model - not claiming fable 5. **Action:** batch plan (6 commands C12-C17; business-system merged into offer; workflow-export+template as arguments; +1 automation-engineer skill). Built 6 command files + 8 specs + 4 extensions + skill + 8 scaffolds. P1: problem-solver + multi-model-planning patched to PASS; /bq-mode pointer. Legacy skill/ (119 files) inventoried -> legacy-marked (skill/README.md); automation refs merged; nothing deleted. High-risk hardening (confidence caps + refusal cases; FTC page 403 -> ECO-labeled honestly). Okay-command tightening (no overbuild). Full sync: ID map, orchestration map (59/31), registry (+automation-engineer, P1 PASS), skill router, README/commands.md/catalog/CLAUDE.md/menus, installers (+8 dirs alpha.24). MASTER: built -> section A; parked = data-product/agent-pack/app-store-kit/cross-agent/heavy-media/provider + doctrine-pack idea. No live trials; no heavy runtime; no provider integration.

## 2026-06-12 - v3.0.0-alpha.23: /bq-offer + tightening pass (Claude Fable 5)

**Action:** Part 1: built C11 /bq-offer per the 15-step feature workflow (command + OFFER_ENGINE spec + offers/ scaffold + installers + ID map + both routers + orchestration map + menu/help/README/commands.md/catalog/CLAUDE.md + proposal sibling notes; 6 queued-markers flipped to built-not-live-tested). Part 2: tightening - best-practice audit (15Q/8 domains; LIVE: AGENTS.md = Linux Foundation standard -> cross-agent docs rewritten; Slidev v52.16.0 current), duplication/conflict (11 checks: 3 fixed, 1 deferred: heavy-era skill/ dir), generic-output risk ranking, quality matrices (53 cmds + 30 skills: 0 weak, 2 thin -> P1), evidence log (LIVE/PRIOR-LIVE/ECO), tightening plan. MASTER: C11 -> section A; alpha.24 uncommitted until live trials. Process note (honest): the finalization script initially failed on an encoding anchor AFTER the main commit fired, so alpha.23 shipped as two commits - the feature commit plus this finalization commit (logs/version/MASTER/AGENTS-patches + temp-script cleanup). Lesson logged to MISTAKE_MEMORY: never chain git after a script with a `;` - use && all the way. Model: Fable 5 - no switch.

## 2026-06-12 — post-alpha.22 maintenance pass (Claude Fable 5)

**Action:** Stabilization close-out. (1) Skill-audit baseline for the 3 new skills — 3/3 PASS structural with live-validation flagged pending (full field-by-field audit in SKILL_AUDIT_ALPHA_22_BASELINE.md; registry Q updated). (2) Drift verification — 2 findings found+fixed (USING runbook 29-skills straggler; catalog 27→29 line), all other checks consistent with command evidence (DRIFT_VERIFICATION_POST_ORCHESTRATION.md). (3) Course PDF READ DIRECTLY this session (5 pages, Arabic, text-extractable — no OCR; pypdf; RTL artifacts identified = localization-rtl first real exercise): 11-stage framework extracted to English in COURSE_PDF_REFERENCE_NOTES.md; COURSE_ENGINE reference policy rewritten from "not accessible" to VERIFIED Reference A; language rule added (concepts in English; output language = user request). (4) REMAINING_WORK_MASTER created as canonical remaining-work ledger; queryability wired into 6 commands + map §14 + model §5; old roadmap ledger superseded-pointed. README sanity clean. NOT done by design: no version bump, no /bq-offer, no parked builds, no live-trial claims. Model: Fable 5 — available, no reroute, no degradation.

## 2026-06-12 — alpha.22 orchestration update (Claude Fable 5)

**Action:** Auto Mode / global orchestration strengthening + per-agent setup docs. Built: bequite-orchestrator skill (#30, global brain) + ORCHESTRATION_MAP (source of truth; 13 sections) + BEQUITE_ORCHESTRATION_MODEL (pipeline/layer table/conflict + missing-capability protocols) + AUTO_MODE_ORCHESTRATION_STRATEGY (15-step anti-skip; one-item-two-buyers canonical example) + SYSTEM_DESIGN_REASONING_STANDARD (mandatory risk block + 6 domain libraries) + CONTEXT_COMPACTION_STRATEGY (40/60/75/85% + Context Compact format) + LOW_COST_MODEL_EXECUTION_STRATEGY (tiers A/B/C; honest gap-narrowing claim) + 4 state rule cards + USING_BEQUITE_WITH_SMALLER_MODELS + INSTALL_FOR_OTHER_AGENTS (universal AGENTS.md bridge; 11 agents x 3 OS x global/per-project). Wired: 9 commands (auto anti-skip; plan/feature/implement risk check; review/verify risk verification; suggest missing-capability; discover/bequite/skill-audit orchestrator refs) + both routers (conflict-to-orchestrator) + strategy pointers. Counts 29-to-30 synced (registry/advisor/README/CLAUDE.md/commands.md/runbooks); installers +4 templates (bash -n OK; ps1 parse 0 errors). NOT done by design: no heavy runtime/provider integrations; no live trials claimed; /bq-offer still queued. Model: Fable 5 (user toggled Opus 4.8 and back before this pass; executed on Fable 5 — reported, not silent).

## 2026-06-12 — alpha.22 stabilization pass (Claude Fable 5)

**Action:** Stabilization/documentation/compatibility pass — NOT a feature build. Repo-reality verification first: 4 audits written (post-release stabilization · scraping context · cross-agent · classification). Key findings: alpha.22 artifacts all verified on disk; 7 doc-drift findings fixed (README 39/42/18-era counts, alpha.8 sections, thin bq-review description, deprecated alias in INSTALL workflow C, missing scraping router signal); scraping/web-automation capability VERIFIED PRESENT and first-class (232-line skill, Article VIII, verified tool catalog incl. Scrapling — name live-verified via WebFetch, v0.4.9 June 2026); the two old strategy/template doc names never existed in v3 — skill ruled canonical carrier, no duplicate docs created. Course Engine gained 12-rule OCR/scanned-PDF Source Intake (+SOURCE_INTAKE_REPORT, 15 outputs). Cross-agent compatibility documented (strategy + runbook + matrix; ~85% portable by construction; adapters = roadmap only). README rewritten as public product docs. Skill Expansion Reality Check: localization-rtl + guard-pass fully documented; 6 capability commands ruled command-only (course-architect skill deferred until first live run). Roadmap ledger created (REMAINING_ROADMAP_TASKS: A–E buckets). NOT done, by design: live trials (none claimed), /bq-offer build (stays queued C11), version bump. Model: Fable 5; no switch/reroute.

## 2026-06-12 — alpha.22 addendum: A→Z verification + Forgotten ChatGPT Candidate Review (Claude Fable 5)

**Action:** User re-sent the alpha.22 prompt for verification + added the 7-candidate review. Verified all acceptance criteria with evidence (artifact existence sweep: 11/11 OK; V3 row count = 20; IDs confirmed in /bequite + /bq-help). Clarified that Discovery V3 (20 ideas) WAS shipped in 5ab5c45 — compaction hid it from chat; restated in report. Created `.bequite/tasks/ALPHA_22_TASK_CHECKLIST.md` (prompt-as-tasks; 6 remaining items). Forgotten-candidate rulings (doc-only, allowed pre-verification): 4 V1 arguments approved (feature demo-data — absorbs V3 #18 · review persona · scope intake · proposal price), 1 merge (demo-script → release demo-video profile), 1 V2 park (release template), `/bq-offer` confirmed for alpha.23 (C11 queued in ID map). Updated: shape decisions, V3 addendum, COMMAND_ROUTER §6 signals, ID map ARG notes, SKILL_ROUTER addendum routes, bq-release profiles note. Model: Fable 5; no switch/reroute.
