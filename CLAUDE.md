...
## Current spec: v3.0.0-alpha.23 — Offer Engine (on the alpha.22 navigation/orchestration base)

- **53 active slash commands + 1 deprecated alias** (`.claude/commands/`) — alpha.23 adds C11 `/bq-offer` (Offer Engine — monetization chain C6→C10→C11→C8→proof; `.bequite/offers/`; spec OFFER_ENGINE.md; built, not live-tested); alpha.22 adds C3 `/bq-reference` · C4 `/bq-knowledge` · C5 `/bq-course` · C6 `/bq-pain-radar` · C7 `/bq-integrate` · C8 `/bq-proposal`; alpha.19 added `/bq-writing-dna` + `/bq-skill-audit`. **Catalog IDs (alpha.22):** display-only W0–W5 / N / O / C / M IDs in `.bequite/commands/COMMAND_ID_MAP.md` — files are NEVER renamed (`docs/architecture/COMMAND_NUMBERING_AND_ORDERING_STRATEGY.md`).
- **30 skills** (alpha.22 adds `bequite-localization-rtl` + `bequite-guard-pass` + `bequite-orchestrator` (global brain — orchestration update); alpha.21 added `bequite-frontier-reasoning-coach`)
