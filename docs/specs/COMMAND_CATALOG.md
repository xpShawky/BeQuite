...
**Total commands:** 60 active + 1 deprecated alias (M3 /bq-hooks added 2026-06-13) (`/bq-add-feature` → `/bq-feature`) — every command carries a stable display-only catalog ID (W0–W5 / N / O / C / M); canonical map `.bequite/commands/COMMAND_ID_MAP.md`
**alpha.24:** 6 capability commands C12 /bq-automation · C13 /bq-local-business · C14 /bq-brand-kit · C15 /bq-community · C16 /bq-recording · C17 /bq-start (specs in docs/specs/); extensions: /bq-offer business-system|agency · /bq-handoff workflow-export · /bq-release template · Course Architect in /bq-course; +1 skill automation-engineer (31); P1 skill patches; legacy skill/ folder legacy-marked; built NOT live-tested
**alpha.23:** **C11 /bq-offer** — Offer Engine (idea/niche → sellable productized offer; 12 outputs in `.bequite/offers/`; honest-selling rules: no invented demand, no fake income claims, promise ≠ guarantee; completes the monetization chain C6→C10→C11→C8→proof; spec `docs/specs/OFFER_ENGINE.md`; built, not live-tested)
**alpha.22:** Command Router layer (`docs/architecture/WORKFLOW_COMMAND_ROUTER.md` — contract step 12 = multi-command Next Command Recommendations; auto mode reports internal workflow by ID) · 6 new capability commands: **C3 /bq-reference** (screenshot|url|flow|style= → design extraction + clone-safe rebuild blueprint, `.bequite/reference/`) · **C4 /bq-knowledge** (build|ask|rag-plan|export, `.bequite/knowledge/`) · **C5 /bq-course** (Course Engine, `.bequite/courses/`, 15 outputs incl. SOURCE_INTAKE_REPORT — handles text/scanned/OCR/RTL PDFs via the 12-rule Source Intake) · **C6 /bq-pain-radar** (ethical public-pain mining, `.bequite/pain-radar/`) · **C7 /bq-integrate** (API blueprint, `.bequite/integrations/`) · **C8 /bq-proposal** (honest proposals, `.bequite/proposals/`) · skills 27 → **30** (localization-rtl + guard-pass; + orchestrator in the same-release orchestration update) · 11 argument workflows (verify regressions|drift · release readiness|announce|proof|demo-video · plan from-issues|migration · scope from-interview · test from-spec|fixtures · handoff client · audit client|a11y · uiux-variants style=)
