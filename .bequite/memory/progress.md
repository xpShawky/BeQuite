...
- **Sub-version:** v0.9.0 just tagged. **21 sub-versions tagged: v0.1.0 / v0.1.1 / v0.1.2 / v0.2.0 / v0.2.1 / v0.3.0 / v0.4.0 / v0.4.1 / v0.4.2 / v0.4.3 / v0.5.0 / v0.5.1 / v0.5.2 / v0.5.3 / v0.6.0 / v0.6.1 / v0.7.0 / v0.7.1 / v0.8.0 / v0.8.1 / v0.9.0.** Counts (git-verified post-v0.9.0, pre-commit): ~26 commits, ~195+ tracked files, ~35k lines added net. (Earlier "30,000+/22,000+/13,000+" estimates were inflated; corrected per Article VI honest reporting.)
...
- **Phases shipped:** v0.1.0 → v0.9.0 inclusive (21 tags). Target: v1.0.0 (full Layer 1 release). Layer 2 Studio: v2.0.0+ (per ADR-008).
- **Open features:** 1 (the BeQuite v1.0.0 build; 5 sub-versions remain to v1.0.0 — v0.9.1 next).
- **Receipt chain integrity:** Schema v1 + ed25519 signing operational. No receipts emitted yet on BeQuite-itself (first phase-transition receipt emits when v0.10.0 wires auto-mode or any user manually invokes `bequite receipts emit`).
- **Receipt schema:** v1 (since v0.7.0; signature field added v0.7.1, additive). ed25519 signing operational since v0.7.1.
...
| v0.7.1 | Signed receipts (`cli/bequite/receipts_signing.py` ed25519 keypair on `bequite init` + `Receipt.signature` additive field + `ReceiptStore.write(sign_with=)` + `bequite verify-receipts [--strict]` + `bequite keygen` + 9-test signing integration suite all passing; 19/19 receipts+signing tests green) | ✅ tagged 2026-05-10 |
| v0.8.0 | Multi-model routing live (`cli/bequite/providers/{anthropic,openai,google,deepseek,ollama}.py` 5 adapters + `AiProvider` Protocol + `Completion` dataclass; `cli/bequite/router.py` with select_route + dispatch + fallback; `cli/bequite/cost_ledger.py` writes `.bequite/cache/cost-ledger.json` so `stop-cost-budget.sh` hook is operational; `bequite route {show,list,providers}` + `bequite ledger {show,reset}` Click groups; 15-test integration suite all passing; combined 34/34 green) | ✅ tagged 2026-05-10 |
| v0.8.1 | Live pricing fetch (best-effort): `cli/bequite/pricing.py` with cache + 24h TTL + offline fallback + WebFetch best-effort live extraction; `skill/references/pricing-table.md` vendored May-2026 snapshot; provider adapters' `estimate_cost_usd()` consult cache before hard-coded fallback; `bequite pricing {show,list,refresh}` Click group; 14-test integration suite; combined 48/48 green | ✅ tagged 2026-05-10 |
| v0.9.0 | Three example projects scaffolded + spec'd (01-bookings-saas: full README + ADR + spec + phases + HANDOFF; 02-ai-tool-wrapper: README + ADR; 03-tauri-note-app: README + ADR with brief reconciliations applied). Honest scope: scaffolds + walkthroughs, not production-quality code (downstream work). | ✅ tagged 2026-05-10 |
| v0.9.1 | E2E test harness (seven-phase walk + auto-mode + doctrine-loading) + `examples-e2e.yml` GitHub Actions workflow | **next** |
...
```
2026-05-10  v0.9.0 tagged — Three example projects (scaffolded + spec'd). examples/README.md (entry point + forking guidance). examples/01-bookings-saas/ (Doctrine default-web-saas; README; full ADR-001 stack with per-rule Doctrine compliance check; spec.md with 4 flows + Skeptic kill-shot for concurrent-booking race; phases.md with 7-phase decomposition; HANDOFF.md with engineer + non-engineer sections). examples/02-ai-tool-wrapper/ (Doctrine cli-tool; README with phases inline; ADR-001 reusing BeQuite providers/ adapters; cost-per-invocation analysis at $0.0006/call). examples/03-tauri-note-app/ (Doctrine desktop-tauri; README with brief reconciliations applied — NOT Stronghold/altool/EV cert/relic; ADR-001 with codesigning matrix notarytool macOS + AzureSignTool Windows + AKV cert 1-yr cap risk). Honest scope (Article VI): scaffolds + walkthroughs, not production code; production work lands via v0.9.1 e2e harness driving bequite auto + downstream example iterations. cli/bequite/__init__.py + cli/pyproject.toml bumped to 0.9.0.

2026-05-10  v0.8.1 tagged — Live pricing fetch (best-effort). cli/bequite/pricing.py (~330 lines): VENDOR_URLS dict for Anthropic/OpenAI/Google/DeepSeek; fetch_vendor_page (httpx GET with 10s timeout + UA "bequite-pricing-probe/0.8.1" + follow_redirects); extract_prices_from_html (coarse regex for paragraph-bounded model+two-prices pairs); cache shape with fetched_utc + ttl_hours + models{model: {input, output, source}} + warnings; pricing_for(model) returns (rates, source) with source ∈ live/stale/fallback/unknown; estimate_cost_usd convenience wrapper. CLI: python -m bequite.pricing {refresh,show,list}. Provider adapters' estimate_cost_usd() updated to consult pricing.pricing_for() first; soft-import preserves graceful degradation. skill/references/pricing-table.md vendored fallback covering 5 model providers + hosting (Vercel/Cloudflare/Render/Fly/Railway) + auth (Clerk/Auth0/Better-Auth/Supabase) + database (Supabase/Neon/Convex). bequite pricing {show,list,refresh} Click group. tests/integration/pricing/test_pricing_smoke.py — 14 tests. Combined integration suite: 48/48 green (10 receipts + 9 signing + 15 router + 14 pricing). cli/bequite/__init__.py + cli/pyproject.toml bumped to 0.8.1.

2026-05-10  v0.8.0 tagged — Multi-model routing live. cli/bequite/providers/__init__.py (AiProvider Protocol + Completion dataclass + REGISTERED_PROVIDERS tuple + get_provider factory). 5 thin adapters: anthropic.py (Claude family + pricing $15/$3/$0.80 per 1M); openai.py (GPT-5/o3 family + pricing); google.py (Gemini via google-genai SDK); deepseek.py (subclasses OpenAIProvider with base_url=api.deepseek.com); ollama.py (httpx + localhost; cost always $0.00). Each adapter graceful-degrades when SDK or API key absent. cli/bequite/router.py: Route dataclass + _provider_for_model heuristic + find_routing_path + select_route (match priority: exact phase+persona → persona+special-phase → phase+orchestrator → catch-all) + dispatch with fallback resolution. cli/bequite/cost_ledger.py: update + read + session_summary + reset_session; writes .bequite/cache/cost-ledger.json (the file stop-cost-budget.sh from v0.3.0 reads — wiring becomes operational). cli/bequite/__main__.py: bequite route {show,list,providers} + bequite ledger {show,reset} Click groups. tests/integration/router/test_router_smoke.py — 15 tests covering provider Protocol conformance + heuristics + route selection + pricing estimates + dispatch + fallback + ledger accumulation + session_summary + dispatch-updates-ledger. All 15 pass on Python 3.14. Combined integration suite (receipts + signing + router): 34/34 green. cli/bequite/__init__.py + cli/pyproject.toml bumped to 0.8.0.

2026-05-10  v0.7.1 tagged — Signed receipts (ed25519). cli/bequite/receipts_signing.py: generate_keypair (private at .bequite/.keys/private.pem mode 0600 best-effort + gitignored, public at .bequite/keys/public.pem mode 0644 + committed; refuses overwrite by default); load_private_key/load_public_key with type-checking; sign_dict (signs over canonical-JSON-without-signature; sidesteps chicken-and-egg); verify_dict (recovers signature + recomputes payload + verifies; returns ok/reason); verify_receipts_directory walks .bequite/receipts/ + reports counts (total/signed_valid/signed_invalid/unsigned) + strict-mode rejects unsigned. Receipt schema additive bump: optional signature field (last position; backward-compatible — v0.7.0 receipts still load + emit + roll-up). ReceiptStore.write(receipt, sign_with=key) opt-in: filename remains v0.7.0 deterministic-from-inputs (allows key rotation without breaking chain pointers). bequite verify-receipts Click command (signature + chain validation; refuses without public key; --strict opt-in). bequite keygen Click command. bequite init auto-generates keypair on scaffold + appends .bequite/.keys/ patterns to project's .gitignore. tests/integration/receipts/test_signing_smoke.py — 9 tests covering keygen lifecycle (creates / refuses-overwrite / explicit-overwrite); sign-verify roundtrip; tampered-body rejected; unsigned-strict-fails; unsigned-lenient-tolerated; ReceiptStore.write(sign_with=) emits signed; cross-paste-signature rejection. All 9 pass on Python 3.14. Combined receipts+signing suite: 19/19 green. cli/bequite/__init__.py + cli/pyproject.toml bumped to 0.7.1.
